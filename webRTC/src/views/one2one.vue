      ...
      <el-col :span="6">
        <div style="width: 100%;height: 800px;">
          <ul v-for="(item, index) in roomUserList" :key="index">
            ...
            <el-tag v-if="userInfo.userId === item.userId" type="danger" size="mini"
              @click="changeBitRate()">增加比特率</el-tag>
            <el-button size="mini" type="primary" v-if="userInfo.userId !== item.userId"
              @click="call(item)">通话</el-button>
            <el-button v-if="userInfo.userId === item.userId" size="mini" type="danger"
...
export default {
  ...
  methods: {
    ...
    init(userId, roomId, nickname) {
      ...
      this.linkSocket = io(this.$serverSocketUrl, { // 建立socket连接
      ...
      this.linkSocket.on("connect", (e) => { // 连接成功
        console.log("server init connect success", that.linkSocket)
      })

      this.linkSocket.on("roomUserList", (e) => { // 更新用户list
      ...
      // msg事件处理两件事情：
      // 1 加入房间，离开房间
      // 2 接收对方的sdp和candidate，为建立webRTC连接做准备
